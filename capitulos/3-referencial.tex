...
que a implantação do software seja feita da forma correta, evitando orose acontecimento
...
de hospagem virtual, que será visto na subseção a seguir.


\subsection{Múltiplas Instâncias com Hospedagem Virtual}
%falar sobre
...
\cite{apachevh} hospedagem virtual é um termo que refere-se à prática de executar
...
tem vários nomes em execução em um único endereço de ip, possibiltiando que em um
...
\begin{lstlisting}[language=Xml,label=dice_index,caption={Exemplo de arquivo de confgiuração de hospedagem virtual no apache}]
  <VirtualHost *:80>
...
direcioandas aos arquivos que estão em /var/www/html. Já no Nginx a configuração
...
\begin{lstlisting}[language=Ruby,label=dice_index,caption={Exemplo de arquivo de confgiuração de hospedagem virtual no Nginx}]
  server {
...
arquivo index que será o arquivo da pagina inicial do nome do servidor na porta 80,
...
porém para este trabalho será necessáro apenas o uso de hospedagem virtual.
...
softwre, porém outras atividades também são importantes e que fogem da responsabilidade
...
segurança. Para entender melhor quais são esses procedimentos, será abordado quais são
...
de pacotes, disfarce na fonte e modifcação e exclusão de mensagem, e para proteger
desses ataques é importnte o uso de comunidação segura, protegendo as aplicações. Para
...
\begin{itemize}
  ...
  os atacantes podem tentar adiquirir os segredos da rede e lançar ataques DoS,
  por isso é necessário uma boa configuração de firewall e e sistemas de detecção
  de invasão.
\end{itemize}
...
e SMTP, assim tornando adicioanndo uma camada de segurança as aplicações que
serão implantadas.
...
esses documentos são padrozindos, como por exemplo o HTML, para que os navegadores
possam interpretar e passar a infomação ao usuário.
...
um com outro a partir de troca de mesagens, o papel do HTTP é definir a estrutura dessas
...
Já o SMTP é protocolo responsável por transfer mensagens de servidores de correio
remetentes para servidores de correio destinatários, também transferindo arquivos,
de um servidor de correio para o outro. Uma diferença diferença entre o HTTP e o SMTP
é que o protocolo HTTP é um protocolo de recuperação de informações, enquanto
o protoclo SMTP é um protocolo de envio de informações \cite{kuroseredes2010}. Isso implica que para um
usuário recuperar as informações de email é necessário o uso de outros componentes,
...
Para adicionar uma camada de segurança, com sigilo e itegridade de dados é necessário
o uso de outro protocolo, o protocolo SSL, para complementar os protocolos HTTP
e SMTP. O protocolo SSL é utilizado por basicamente todos os sites, como Googlem
...
dados transmitidos sejam sigilosos e seguros \cite{kuroseredes2010}. Para poder utilizar o SSL é necessaŕio
...
\subsection{Assinaturas e Ceritificados Digitais}
...
auto-assinados, que são os certificados gerandos por conta própria através de
ferramentas, uma exemplo de ferramenta é a ferramenta OpenSSL, que permite requisitar,
 assinar, gerar, exportar e converter certificados digitais.

Um problema em utilizar certificados auto-assinados é que os clientes precisam
